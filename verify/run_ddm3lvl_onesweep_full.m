% RUN_DDM3LVL_ONESWEEP_FULL  Execute the source LOD-DDM one-shot trend sweep.
...
diaryPath = fullfile(outDir, 'onesweep_noh01_full_run.diary.log');
...
setenv('DDM3LVL_OUTPUT_SUFFIX', 'onesweep_noh01_full');
...
if isempty(workerText)
    ...
    if ~isfinite(workers) || workers < 1
        error('run_ddm3lvl_onesweep_full:workers', ...
...
if pool.NumWorkers ~= workers
    error('run_ddm3lvl_onesweep_full:poolSize', ...
        'Expected %d workers, found %d.', workers, pool.NumWorkers);
end

fprintf('Started one-shot source LOD-DDM trend sweep without H0=1 at %s.\n', ...
    char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss')));
run(fullfile(repoRoot, 'verify', 'verify_ddm3lvl_lod_helmholtz_experiments.m'));
fprintf('Finished one-shot source LOD-DDM trend sweep without H0=1 at %s.\n', ...
    char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss')));
