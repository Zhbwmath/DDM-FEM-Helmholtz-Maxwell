% RUN_DDM3LVL_INNERGMRES_TAIL_LOWMEM  Finish the interrupted k=64, H0=1/8 tail.
...
diaryPath = fullfile(outDir, 'innergmres_lu_tail_lowmem_run.diary.log');
...
setenv('DDM3LVL_OUTPUT_SUFFIX', 'innergmres_lu_tail_lowmem');
...
if isempty(workerText)
    ...
    if ~isfinite(workers) || workers < 1
        error('run_ddm3lvl_innergmres_tail_lowmem:workers', ...
...
if pool.NumWorkers ~= workers
    error('run_ddm3lvl_innergmres_tail_lowmem:poolSize', ...
        'Expected %d workers, found %d.', workers, pool.NumWorkers);
end

fprintf('Started low-memory inner-GMRES source LOD-DDM tail at %s.\n', ...
    char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss')));
run(fullfile(repoRoot, 'verify', 'verify_ddm3lvl_lod_helmholtz_experiments.m'));
fprintf('Finished low-memory inner-GMRES source LOD-DDM tail at %s.\n', ...
    char(datetime('now', 'Format', 'yyyy-MM-dd HH:mm:ss')));
