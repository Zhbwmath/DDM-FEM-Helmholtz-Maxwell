...
Mref = phi1d' * (w1d(:) .* phi1d);

maxBd = 3 * size(elem, 1);
nEdgeDof = degree + 1;
nEntries = maxBd * nEdgeDof^2;
ii = zeros(nEntries, 1);
jj = zeros(nEntries, 1);
ss = zeros(nEntries, 1);
idx = 0;
...
%     edge1 (v2,v3): DOFs [2, 5, 3]   (v2, e23, v3)
%     edge2 (v3,v1): DOFs [3, 6, 1]   (v3, e31, v1)
%     edge3 (v1,v2): DOFs [1, 4, 2]   (v1, e12, v2)
%
%   P3 ordering: v1,v2,v3, e12a,e12b, e23a,e23b, e31a,e31b, centroid
%     edge1 (v2,v3): DOFs [2, 6, 7, 3]      (v2, e23a, e23b, v3)
%     edge2 (v3,v1): DOFs [3, 8, 9, 1]      (v3, e31a, e31b, v1)
%     edge3 (v1,v2): DOFs [1, 4, 5, 2]      (v1, e12a, e12b, v2)

switch degree
    case 2
        switch edgeK
            case 1, dofIdx = [2, 5, 3];    % edge (v2,v3)
            case 2, dofIdx = [3, 6, 1];    % edge (v3,v1)
            case 3, dofIdx = [1, 4, 2];    % edge (v1,v2)
        end
    case 3
        switch edgeK
            case 1, dofIdx = [2, 6, 7, 3]; % edge (v2,v3): near v2 then v3
            case 2, dofIdx = [3, 8, 9, 1]; % edge (v3,v1): near v3 then v1
            case 3, dofIdx = [1, 4, 5, 2]; % edge (v1,v2): near v1 then v2
        end
    otherwise
